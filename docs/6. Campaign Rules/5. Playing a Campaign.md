...
Before any games are played, if the current Pit Fighter is part of a warband, then a Pit Fight occurs. Once the finale has begun, any results of 'Pit Fight' or 'Light Wound' on the Hero Scars table are treated as 'Nothing Happens' instead.

### Penultimate Battle

First, if there is more than one player in a Faction, a single regular battle is played between all players of the same Faction. The winner of that battle is then chosen as the representative of the Faction in the finale Special Scenario. No Faction Points for having fewer wins are given in this battle, so players with more wins will generally have an advantage over those that don't.

For these battles, a standard 'dense city' map should be used and the deployment types should be limited to 'Edge deployment' or 'Corner deployment' (reroll if another deployment type is rolled). In addition, as there is no incentive to voluntarily rout, the third Rout Check onwards that a player makes within this battle will be made with Disadvantage. No Treasure tokens are placed down during this battle, so more units will be Incapacitated before Rout Checks are made.
...
Maybe faction points are actually reversed in this game, so the player with more wins gets more points? But maybe both sides do indeed get the usual free 50 Faction Points, but there are no special rules? I want to make sure the side with more wins is favoured, but I don't want it to be certain defeat for the player with more losses, otherwise there is no point in having the game at all.
-->

For the final Special Scenario, the final player of each faction face off in a 1v1v1. The exact scenario should have been chosen at the start of the campaign. The winner of this Special Scenario is the overall winner of the campaign!
