...
3. Units Fall after failing an Agility Check of some kind (such as Climbing Up, Climbing Down, Jumping Across, or Jumping Down) and fail their Mishap Roll

As you can see, some Injuries are inflicted by Injury Checks and others by Mishap Rolls. Understanding the difference is important.
...
Injury modifiers modify the value of the d20 roll made when checking for Injury. That means it is modifying the Melee, Ranged, or Cast Check roll. This is done for the purposes of the Injury checking only, and does not determine whether the Melee, Ranged, or Cast Check itself succeed (those are instead modified by Melee, Ranged, or Casting modifiers, as you would expect).

Injury modifiers that are part of a unit's stat block only apply to Melee Attacks, while modifiers that are part of weapons are only applicable to attacks made with that weapon. This modifiers can stack (so a Hammer with +4 Inj and a base Inj modifier of +2 would give a total Inj modifier of +6).

### Mishap Rolls

Sometimes the rules call for an 'Mishap Roll', which works differently to calculating Injury Checks. This Roll is called for when resolving Falling, for some Spells, and for some other instances.

To make an Mishap Roll, simply roll a d20 and consult the Mishap Roll Table below. Note that a 1-11 has no effect. This means that, unlike an Injury Check, there is no guarantee that a target will be Injured at all.
...
As Falling is the most common way for an Mishap Roll to be made, the calculation is included here for reference.

When a unit Falls, roll on the Mishap Roll Table. For every extra full inch above 2” that the unit Falls from, add a +2 modifier.
...
A Stunned unit cannot do anything. It is represented by placing the unit's model on its side, face down.
...
A Dazed unit cannot do anything except move 1". It is represented by placing the unit's model on its side, face up.
...
If the unit fails, then it Falls, landing at the bottom of the drop at a place where it is not 1" from another edge. If it would be somewhere within 1" of a ledge, it instead keeps falling. Add the total height together and make single roll.
...
## Units with Multiple Wounds

All previous rules for checking for Injury and for Mishap Rolls have made the assumption that the unit only has 1 Wound. However, some units (especially heroes) will have more than 1 Wound.

Wounds essentially create a buffer for a unit before they can become Injured. At any point that a unit would become Injured, it instead loses 1 Wound first (this includes from things such as Falling or when a Spell immediately Dazes a unit). If this would bring its Wounds to 0, then it is injured as normal.

So a unit with 3 Wounds could lose 2 Wounds instead of making a check for Injury, or if they would otherwise have been Injured from an Mishap Roll. After that, they will need to make Injury Checks or Rolls as normal.
