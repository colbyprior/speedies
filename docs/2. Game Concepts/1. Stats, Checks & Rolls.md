...
This can include 'Mishap Rolls', 'Scar Rolls', 'Out-of-Sight Rolls', and 'Loot Rolls'. Whenever you read 'Roll', you know that you will need to consult a table to determine the outcome.
...
If a player rolls a '20' during a non-attack Check, the Check will always succeed, regardless of difficulty or other modifiers.
...
If, as part of any injury roll, the modified value of that roll is 20 or higher, it becomes a Critical Hit! This causes the die to 'explode'. Continue to resolve the current attack as normal. In addition, attempt and resolve an additional attack for free against the same target. This additional attack cannot also become a Critical Hit.

For example: Alice rolls a 16 for one of her unit's Melee Attacks. This unit has a Hammer with a +4 modifier to Injury. That means the modified Injury Roll is 20, so it is a Critical Hit! She makes an extra attack and rolls a 20! She does not gain an additional attack from the roll of the second 20. The target must now make Defence Checks against each attack, or be Incapacitated.
