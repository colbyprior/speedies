...
Picking up Treasure tokens is done at the start of each turn and only if there are no enemies nearby. Units can only carry one Treasure token and may not Run while doing so. Picking up Treasure tokens can be fraught with danger, as they may also trigger Events or Blight Checks.

When the game ends, players also check to see if their warbands pick up any remaining Treasure, in a [last effort before retreating](/Game%20Rules/Upkeep%20Phase). The winner of the game is more likely to pick up Treasure in this way.
## Income & Loot
...
Particularly good rolls mean that the Treasure contains Loot in addition to Income. Loot comes in the form of Rare Consumables and Rare Items, both of which can be equipped to heroes in future games.
