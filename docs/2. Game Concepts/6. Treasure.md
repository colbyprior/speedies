...
Picking up Treasure tokens is done at the start of each turn. If an enemy unit is contesting it by standing nearby, the unit must first pass a Morale Check to pick up the Treasure. Units can only carry one Treasure token and may not Run while doing so. Picking up Treasure tokens can be fraught with danger, as they may also trigger Events or Blight Checks.
