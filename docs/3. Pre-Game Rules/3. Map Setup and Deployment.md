...
| d20 | Map Type                                           |
| --- | -------------------------------------------------- |
| 1   | Tower in the centre                                |
| 2   | Village alongside a river                          |
| 3   | Main street in the city                            |
...
| 9   | Canals                                             |
| 10  | Sewers                                             |
| 11  | Open space in the middle (town square, park, etc.) |
| 12  | Untouched city (no bridge connections)             |
| 13  | Abandoned hideout (many bridge connections)        |
| 14  | Crude wooden fort in centre                        |
| 15  | Dense city                                         |
| 16  | City outskirts (low density)                       |
| 17  | City walls (across board)                          |
| 18  | City with keep in the middle                       |
| 19  | Warehouse district                                 |
| 20  | Mansion (combine multiple buildings)               |
...
It's recommended to place Treasure tokens with one directly in the middle and the other four spread out so that they are all a third of the board away from each board edge, as well as a third away from each other. Having them be at a variety of heights is ideal.
...
#### Treasure Token Pools

When placing Treasure Tokens, they are chosen from a pool that is one larger in size than the number being placed. This is to prevent players from generally being able to determine the type of the last Treasure Token on the board.

For most games 5 Tokens are placed, which means a pool of 6 Tokens is required. There are some exceptions for the first few games of a campaign, as described later. You can use the table below to determine what the pool of Tokens should look like, depending on the number of Tokens placed.

| Tokens Placed | Total Tokens in Pool | Regular Tokens | Blight Tokens | Event Tokens |
| ------------- | -------------------- | -------------- | ------------- | ------------ |
| 5             | 6                    | 3              | 2             | 1            |
| 6             | 7                    | 4              | 2             | 1            |
| 7             | 8                    | 5              | 2             | 1            |
...
When placing them, they should be chosen randomly and blindly. They should be placed face down and revealed the first time a unit picks up the Treasure token. If the Treasure token is picked up again, nothing happens.

#### First Games of the Campaign

At the start of the campaign, Blightmeer is largely unlooted, so there is an abundance of treasure! However, as the campaign progresses, treasure becomes harder to find.
...
| d20   | Deployments                                                                                                                                                  |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1-6   | Team A anywhere along one edge, Team B anywhere along opposite edge                                                                                          |
| 7-12  | Team A in one corner, Team B in opposite corner                                                                                                              |
| 13-14 | Half of Team A in one corner, other half in an adjacent corner. Team B is split in half across two remaining corners.                                        |
| 15-16 | Half of Team A in one corner, other half in opposite corner. Team B is split in half across two remaining corners.                                           |
| 17-18 | Half of Team A in middle point of one edge, other half in middle point of opposite edge. Team B is split in half across middle point of two remaining edges. |
| 19-20 | Team A in centre, Team B along all edges (whoever rolls to deploy first is Team A)                                                                           |
...
For a player to deploy, they do so as described in the Deployment Type that was rolled. Unless stated otherwise, units deployed along the edge of the board must not deploy any more than 2" away from the edge. The intention is that warbands may be deployed up to two units deep, but no more. If units are deployed at a point (eg. in a corner or the centre of the board), they must not deploy any more than 5" away from that point, unless stated otherwise.

If warbands are split into halves or more groups, they must be split as evenly as possible by unit count. Whether a unit is a Hero or Henchman does not matter.
