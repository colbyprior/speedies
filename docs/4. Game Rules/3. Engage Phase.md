...
3.  Determine the distance your unit is able to move during the Engage
	1. If your unit can see the target, the distance is equal to the unit's Run speed (ie. 3" more than the base Move of the unit)
...
7. If the distance in Step 3 is not enough, the Engage fails. Move the unit toward the target via the chosen path, but only as far as the unit's base Move attribute (or only half of the unit's base Move attribute if a 1-5 was rolled on an [Out-of-Sight Roll](#out-of-sight-rolls)). If the place the unit stops is midway through a Jump Across check, it Falls.
...
When attempting to Engage a unit that your unit does not have Line-of-Sight of, roll on this table. On a roll of 16-20, the Engage distance is as normal (ie. base Move of the unit +3"). On any other roll, it is reduced as described.
...
| 16-20 | Engage as normal           | 1x base Move of the unit   |
...
## Jump Attack
A unit has performed a Jump Attack if it made a successful Jump Down check as part of the Engage and landed in base contact with the enemy (ie. did not land and then move into base contact afterwards).

When Jumping Down as part of an Engage, a unit cannot be Intercepted. If it lands and then continues moving afterwards to complete an Engage, it can be Intercepted via the usual rules from that point on.

If a unit successfully performs a Jump Attack, that unit roll its attacks with Advantage for the current turn.

It's assumed a unit will always attempt to Jump Attack as part of a Jump Down as long as it has the jump range to do so. However, if it fails the Jump Down, it will still land in base contact with the enemy and count as being Engaged. This means that, if it was Dazed or Stunned by the Fall, it may immediately be Incapacitated by the enemy in the upcoming Melee Phase. If it avoided being injured from Falling, it will fight as normal but will not get the benefits of a successful Jump Attack.
