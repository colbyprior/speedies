...
A Melee Combat is resolved by first checking for Fear, if neccessary, then the attack order of the combat. After that, the attack order is resolved. This is done by going through and resolving each unit's attacks in order. To do that, player's first declare the targets of their unit's Melee Attacks, then resolve those attacks.
...
On the turn a unit Engages or is Engaged by an enemy that causes Fear, that unit must make a Morale Check. On a failure, the unit is Staggered for the remainder of the turn (eg. the unit will have Disadvantage on any Melee or Defence Checks for the combat).

If a unit with Fear Intercepted an enemy unit that was trying to Engage another unit, that enemy unit must make a Fear Check as if it was trying to Engage the unit with Fear.

If both units cause Fear, this negates Fear's effect and neither must make any Morale Checks.

This step is skipped for the combat if no new Engages have occurred this turn against or by units in the combat that cause Fear.

### Determine Attack Order

The next step of resolving a Melee Combat is determining the attack order of all units Engaged in the combat. Dazed or Stunned units are not a part of the attack order. The attack order is as follows:
...
1. Perform a number of Melee Checks, at the same time, equal to the number of Melee Attacks you declared against the target (remember the values!)
	...
	3. If, after Injury modifiers, the roll would be a 20 or higher, you score a Critical Hit. Make an additional Attack and resolve it (from Step 1 onwards). This additional attack cannot trigger further Critical Hits.
...
4. If this lowers their Wounds to 0, determine how Injured the opponent is
	1. Start with the pool of all successful Melee Attacks
		1. For every Wound above 1 that the enemy had, randomly select which successful Melee Attack caused the wound, then remove that attack from the pool
