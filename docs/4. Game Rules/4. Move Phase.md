...
Units that can Move may instead Run, unless that unit is carrying a Treasure token. When Running, a unit may move 3" further than a normal Move (eg. if a unit has 6" Move they may run 9"). A unit that Runs follows all of the same rules as for a regular Move.
...
Make a Mishap Roll. For every extra full inch above 2” that the unit falls from, add +2 to the Mishap Roll.
...
## Jumping Down
Instead of Climbing a unit may Jump Down from a higher point for no vertical movement cost. To Jump Down, place the unit on the ground within 3" (or within its 'jump across' distance if that is greater than 3") of where it is Jumping Down from. This allows units to Jump over other units or terrain, assuming the unit jumping down did so from at least 2" above them.

Now an Agility Check must be made to see whether the unit safely landed. On a success, it lands and may continue Moving as normal. If it fails if Falls, stopping where it landed. Resolve Falling as normal.
